...
class DataValidator:
    ...
    def validate_numeric_range(self, df: pd.DataFrame, column_name: str, 
        ...
        if out_of_range == 0:
            return True, f"Column '{column_name}': All values in range [{min_val}, {max_val}]"
        else:
            return False, f"Column '{column_name}': {out_of_range} values out of range"
    
    def validate_categorical_values(self, df: pd.DataFrame, column_name: str, 
                                   allowed_values: List) -> Tuple[bool, str]:
        ...
        if invalid_values == 0:
            return True, f"Column '{column_name}': All values in allowed set {allowed_values}"
        else:
            # Get actual invalid values
            actual_invalid = df[~df[column_name].isin(allowed_values)][column_name].unique()[:5]
            return False, f"Column '{column_name}': {invalid_values} invalid values. Examples: {actual_invalid.tolist()}"
    ...
    def validate_data(self, data_path: str) -> Dict[str, Any]:
        ...
        print(f"Columns ({len(df.columns)}): {list(df.columns)[:10]}..." if len(df.columns) > 10 else f"Columns: {list(df.columns)}")
        ...
        for col, range_dict in self.rules["numeric_ranges"].items():
            passed, message = self.validate_numeric_range(df, col, 
                                                         range_dict["min"], 
                                                         range_dict["max"])
        ...
        if "default_payment_next_month" in df.columns:
            ...
            self._log_result(results, "target_distribution", "default_payment_next_month", True, message)
            print(f"   📊 {message}")
    ...
    def _log_result(self, results: Dict, check_type: str, column: str, 
    ...
    def _save_results(self, results: Dict, data_path: str):
        ...
        with open(summary_file, "w") as f:
            ...
            f.write(f"Status: {'PASS' if results['summary']['all_passed'] else 'FAIL'}\n")
            
            if not results["summary"]["all_passed"]:
                f.write("\nFailed Checks:\n")
                for validation in results["validations"]:
                    if not validation["passed"]:
                        f.write(f"  - {validation['column']}: {validation['message']}\n")
