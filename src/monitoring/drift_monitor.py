...
class DriftMonitor:
    def __init__(self, api_url="http://localhost:8000", train_data_path="data/processed/train.csv"):
    ...
    def monitor_drift(self, new_data, sample_size=1000):
            ...
            if feature in new_stats['numerical']:
                ...
                if len(train_vals) > 0 and len(new_vals) > 0:
                    ...
                    drift_report['feature_drift'][feature] = {
                        ...
                        'train_mean': float(self.train_stats['numerical'][feature]['mean']),
                        'new_mean': float(new_stats['numerical'][feature]['mean']),
                        'mean_diff': float(abs(self.train_stats['numerical'][feature]['mean'] - 
        ...
        if drift_report['feature_drift']:
            psi_scores = [v.get('psi', 0) for v in drift_report['feature_drift'].values() 
                         if 'psi' in v]
            if psi_scores:
        ...
        try:
            ...
            for _, row in sample_data.iterrows():
                try:
                    ...
                    request_data = row.drop('default_payment_next_month', errors='ignore').to_dict()
    ...
    def generate_report(self, new_data_path, output_dir="reports/monitoring"):
        ...
        full_report = {
            ...
            'summary': {
                ...
                'high_severity_alerts': len([a for a in drift_report['alerts'] 
                                           if a['severity'] == 'HIGH']),
    ...
    def plot_drift_summary(self, drift_report, output_dir):
        ...
        if psi_values:
            ...
            plt.axhline(y=0.1, color='orange', linestyle='--', alpha=0.7, label='Warning threshold (0.1)')
            plt.axhline(y=0.2, color='red', linestyle='--', alpha=0.7, label='Alert threshold (0.2)')
